...
const CustomTooltipContent = props => {
    if (props.payload !== null && props.payload[0] != null) {
        const total = (parseFloat(props.payload[0].payload.cdn) + parseFloat(props.payload[0].payload.p2p)).toFixed(2);
        let spikeReduction = 0;

        if (parseFloat(props.payload[0].payload.p2p) > 0) {
            spikeReduction = (((parseFloat(props.payload[0].payload.p2p) - parseFloat(props.payload[0].payload.cdn)) / parseFloat(props.payload[0].payload.p2p)) * 100).toFixed(2);
        }
